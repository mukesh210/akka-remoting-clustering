...
import part3_clustering.ClusteringExampleDomain.{ProcessFile, ProcessLine, ProcessLineResult}
...
class ClusterWordCountPriorityMailbox(settings: ActorSystem.Settings, config: Config)
  extends UnboundedPriorityMailbox(
    PriorityGenerator{
      case _: MemberEvent => 0  // 0 means highest priority
      case _ => 4
    }
  )
...
class Master extends Actor with ActorLogging {
  ...
  def handleClusterEvent: Receive = {
    ...
    case m: MemberEvent =>
      log.info(s"Another MemberEvent I don't care about: ${m}")
  }
